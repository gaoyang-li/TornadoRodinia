...
public class Lud {
    ...
    public static void lud_diagonal(double[] a, int size, int offset) {
        int i, j, k;
        for (i = 0; i < 16; i++) {
            ...
            double temp = 1 / a[offset * size + i * size + i + offset];
            for (j = i + 1; j < 16; j++) {
                for (k = 0; k < i; k++) {
    ...
    public static void lud(double[] a, int size) {
    ...
    public static void matrix_duplicate(double[] src, double[] dst) {
        for (int i = 0; i < matrix_dim * matrix_dim; i++) {
            dst[i] = src[i];
        }
    }

    public static void lud_verify(double[] m, double[] lu) {
    ...
    public static void create_matrix(double[] mp, int size) {
        double lambda = -0.001;
    ...
    public static void create_matrix_from_file(double[] mp, String input_file) {
        try {
            Scanner scanner = new Scanner(new File(input_file));
            scanner.nextInt();
            for (int i = 0; i < matrix_dim * matrix_dim; i++) {
                mp[i] = scanner.nextDouble();
            }
            scanner.close();
    ...
    public static void print_martix(double[] m) {
        for (int i = 0; i < matrix_dim; i++) {
            for (int j = 0; j < matrix_dim; j++)
