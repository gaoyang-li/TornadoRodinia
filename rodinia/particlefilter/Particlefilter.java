...
public class Particlefilter {
    ...
    public static void setWeights(DoubleArray weights) {
        for (@Parallel int x = 0; x < weights.getSize(); x++) {
            weights.set(x, 1 / ((double)(weights.getSize())));
        }
    }

    public static void setArrayXY(DoubleArray arrayX, DoubleArray arrayY, DoubleArray xe, DoubleArray ye) {
        for (@Parallel int x = 0; x < arrayX.getSize(); x++) {
            arrayX.set(x, xe.get(0));
            arrayY.set(x, ye.get(0));
        }
    }

    public static void updateArrayXY(DoubleArray arrayX, DoubleArray arrayY, VectorInt seed) {
        for (@Parallel int x = 0; x < arrayX.getSize(); x++) {
    ...
    public static void computeLikelihood(DoubleArray arrayX, DoubleArray arrayY, VectorDouble objxy, VectorInt ind, VectorDouble likelihood, Int4 paras, VectorInt I) {
        for (@Parallel int x = 0; x < arrayX.getSize(); x++) {
            for (int y = 0; y < (objxy.size() / 2); y++) {
                int indX = (int)(roundDouble(arrayX.get(x)) + objxy.get(y * 2 + 1));
                int indY = (int)(roundDouble(arrayY.get(x)) + objxy.get(y * 2));
                int absResult = (indX * paras.getY() * paras.getZ() + indY * paras.getZ() + paras.getW());
    ...
    public static void normaliseWeights(DoubleArray weights, DoubleArray sumWeights) {
        for (@Parallel int x = 0; x < weights.getSize(); x++) {
            weights.set(x, weights.get(x) / sumWeights.get(0));
        }
    }

    public static void moveObjectXe(DoubleArray arrayX, DoubleArray weights, @Reduce DoubleArray xe) {
        for (@Parallel int x = 0; x < weights.getSize(); x++) {
            xe.set(0, xe.get(0) + arrayX.get(x) * weights.get(x)); // xe += arrayX.get(x) * weights.get(x);
        }
    }

    public static void moveObjectYe(DoubleArray arrayY, DoubleArray weights, @Reduce DoubleArray ye) {
        for (@Parallel int x = 0; x < weights.getSize(); x++) {
            ye.set(0, ye.get(0) + arrayY.get(x) * weights.get(x)); // ye += arrayY.get(x) * weights.get(x);
        }
    }
    ...
    public static void findNewArray(VectorDouble u, DoubleArray arrayX, DoubleArray arrayY, VectorDouble CDF, VectorDouble xj, VectorDouble yj) {
    ...
    public static void resetWeights(DoubleArray arrayX, DoubleArray arrayY, VectorDouble xj, VectorDouble yj, DoubleArray weights) {
        for (@Parallel int x = 0; x < weights.getSize(); x++) {
    ...
    public static void particleFilter(VectorInt I, int IszX, int IszY, int Nfr, VectorInt seed, int Nparticles) {
        ...
        DoubleArray ye = new DoubleArray(1);
        xe.set(0, roundDouble(IszY / 2.0));
        ye.set(0, roundDouble(IszX / 2.0));
        ...
        DoubleArray arrayY = new DoubleArray(Nparticles);
