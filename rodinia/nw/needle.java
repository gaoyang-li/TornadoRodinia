...
//    static int[] input_itemsets;
//    static int[] output_itemsets;
//    static VectorInt paras;
    static int[][] blosum62 = {
    ...
    public static void usage(String[] args) {
        System.out.println("Usage: needle <max_rows/max_cols> <penalty>\n");
    ...
    public static void nw_optimized1(VectorInt input_itemsets, VectorInt output_itemsets, VectorInt referrence, VectorInt paras) {
        //int transfer_size = max_rows * max_cols;
        for (int blk = 1; blk <= (paras.get(1) - 1) / BLOCK_SIZE; blk++) {
            for (@Parallel int b_index_x = 0; b_index_x < blk; ++b_index_x) {
                ...
                for (@Parallel int i = 0; i < BLOCK_SIZE; ++i) {
                    for (@Parallel int j = 0; j < BLOCK_SIZE; ++j) {
                        reference_l[i * BLOCK_SIZE + j] = referrence.get(paras.get(1) * (b_index_y * BLOCK_SIZE + i + 1) + b_index_x * BLOCK_SIZE + j + 1);
                    }
                }
    ...
    public static void nw_optimized2(VectorInt input_itemsets, VectorInt output_itemsets, VectorInt referrence, VectorInt paras) {
        for (int blk = 2; blk <= (paras.get(1) - 1) / BLOCK_SIZE; blk++) {
            for (int b_index_x = blk - 1; b_index_x < (paras.get(1) - 1) / BLOCK_SIZE; ++b_index_x) {
                int b_index_y = (paras.get(1) - 1) / BLOCK_SIZE + blk - 2 - b_index_x;
                int[] input_itemsets_l = new int[(BLOCK_SIZE + 1) * (BLOCK_SIZE + 1)];
                int[] reference_l = new int[BLOCK_SIZE * BLOCK_SIZE];
                ...
                // Copy input_itemsets to local memory
                for (int i = 0; i < BLOCK_SIZE + 1; ++i) {
                    for (@Parallel int j = 0; j < BLOCK_SIZE + 1; ++j) {
                        input_itemsets_l[i * (BLOCK_SIZE + 1) + j] = input_itemsets.get(paras.get(1) * (b_index_y * BLOCK_SIZE + i) + b_index_x * BLOCK_SIZE + j);
                    }
                }
        ...
        VectorInt output_itemsets = new VectorInt(max_rows * max_cols);
        ...
        TaskGraph taskGraph1 = new TaskGraph("s1")
                .transferToDevice(DataTransferMode.EVERY_EXECUTION, input_itemsets, output_itemsets, reference, paras)
                .task("t1", needle::nw_optimized1, input_itemsets, output_itemsets, reference, paras)
                .task("t2", needle::nw_optimized2, input_itemsets, output_itemsets, reference, paras)
                .transferToHost(DataTransferMode.EVERY_EXECUTION, input_itemsets, output_itemsets, reference, paras);
        ImmutableTaskGraph immutableTaskGraph1 = taskGraph1.snapshot();
        TornadoExecutionPlan executor1 = new TornadoExecutionPlan(immutableTaskGraph1)
                .withDevice(device);
        executor1.execute();
        //nw_optimized(input_itemsets, output_itemsets, reference, paras);
        nw_optimized2(input_itemsets, output_itemsets, reference, paras);
