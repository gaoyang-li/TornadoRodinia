...
public class Hotspot3D {
    ...
    /* Exception in thread "main" uk.ac.manchester.tornado.api.exceptions.TornadoInternalError: unimplemented: address origin unimplemented: org.graalvm.compiler.nodes.ValuePhiNode
            ...
            for (z = 0; z < nz; z++) {
                for (y = 0; y < ny; y++) {
                    for (x = 0; x < nx; x++) {
                        ...
                        tOut.set(c, tIn.get(c) * cc + tIn.get(n) * cn + tIn.get(s) * cs + tIn.get(e) * ce + tIn.get(w) * cw + tIn.get(t) * ct + tIn.get(b) * cb + (dt / Cap) * pIn.get(c) + ct * amb_temp);
                    }
                }
            }
            VectorFloat temp = tIn;
            tIn = tOut;
            tOut = temp;
            i++;
        }
...
//                        .withDevice(device);
                executor1.execute(); //parallel(nx, ny, nz, cc, cw, ce, cs, cb, ct, cn, dt, Cap, tOut_t, tIn_t, pIn);
                VectorFloat t = tIn_t;
                tIn_t = tOut_t;
                tOut_t = t;
                count++;
    ...
    public static void main(String[] args) {
        ...
        VectorFloat powerIn, tempOut, tempIn, tempCopy; // *pCopy;
        ...
        computeTempCPU(powerIn, tempIn, tempOut, numCols, numRows, layers, Cap, Rx, Ry, Rz, dt, iterations);
